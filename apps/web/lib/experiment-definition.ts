...
export type ExperimentDefinitionResult =
  { ok: true; definition: ExperimentDefinition } | { ok: false; errors: string[] }
...
function validListPredicate(value: unknown): value is ExactSegmentScalar[] {
  if (!Array.isArray(value) || value.length < 1 || value.length > MAX_EXPERIMENT_PREDICATE_VALUES)
    return false
...
function parseMetric(value: unknown, path: string, errors: string[]): ExperimentMetric | null {
  ...
  if (!validEventName(value.event)) {
    errors.push(
      `${path}.event must be a non-blank event name up to ${MAX_EXPERIMENT_EVENT_LENGTH} characters`
    )
  }
...
export function parseExperimentDefinition(input: unknown): ExperimentDefinitionResult {
  ...
  if (!validBoundedText(input.hypothesis, MAX_EXPERIMENT_DESCRIPTION_LENGTH, true)) {
    errors.push(
      `definition.hypothesis must be a non-blank string up to ${MAX_EXPERIMENT_DESCRIPTION_LENGTH} characters`
    )
  }
  if (
    typeof input.assignmentEntityType !== 'string' ||
    ...
    if (input.eligibility.tags !== undefined) {
      if (!isRecord(input.eligibility.tags)) {
        ...
        if (entries.length > EXACT_SEGMENT_TAG_FIELDS.length) {
          errors.push(
            `definition.eligibility.tags may contain at most ${EXACT_SEGMENT_TAG_FIELDS.length} predicates`
          )
        }
        tags = {}
  ...
  if (
    ...
    input.variants.length > MAX_EXPERIMENT_VARIANTS
  ) {
    ...
    input.variants.forEach((raw, index) => {
      ...
      if (!validateExperimentKey(raw.key)) {
        errors.push(
          `${path}.key must be 1-64 lowercase characters using letters, numbers, hyphen or underscore`
        )
  ...
  if (!isRecord(input.plannedWindow)) {
    ...
    if (start === null)
      errors.push('definition.plannedWindow.startAt must be a real timestamp with an explicit offset')
    if (end === null)
      errors.push('definition.plannedWindow.endAt must be a real timestamp with an explicit offset')
