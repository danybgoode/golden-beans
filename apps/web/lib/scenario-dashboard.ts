 ...
 */
export async function getScenarioDashboardView(
  projectId: string
): Promise<ScenarioDashboardView> {
  const supabase = getSupabaseServiceClient()
  const [targetsResult, registriesResult, versionsResult, runsResult, securityResult, impactResult,
    policiesResult, statesResult, tripsResult] = await Promise.all([
    supabase
  ...
  const results = [
    targetsResult, registriesResult, versionsResult, runsResult, securityResult, impactResult,
    policiesResult, statesResult, tripsResult,
  ]
  ...
  const versions = new Map(
    boundedRows(versionsResult.data).map((row) => [String(row.id), row])
  )
  const targetKeys = new Map(targets.map((row) => [String(row.id), String(row.key)]))
  const states = new Map(
    boundedRows(statesResult.data).map((row) => [String(row.policy_id), row])
  )
  ...
  return {
    ...
    securityResults: boundedRows(securityResult.data).map((row) => ({
      ...
      observedStatuses: Array.isArray(row.observed_statuses)
        ? row.observed_statuses.map(Number)
        : [],
