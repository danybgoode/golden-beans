...
--
-- ── APPROVED SCHEDULER EXEMPTION — see AGENTS.md § "The scheduler exemption" ──────────────────
-- This function is deliberately cross-tenant. That is permitted ONLY by the narrow, registered
-- exemption Daniel approved 2026-07-22, which scopes the tenancy invariant to REQUEST-DERIVED read
-- paths. Read AGENTS.md for the six conditions and the explicit list of what it does NOT permit;
-- this comment does not grant anything and cannot amend the rule.
--
-- The condition most easily broken by a later edit is #2: this function must return ONLY bare
-- project_ids. Adding any tenant DATA (a name, a count, a timestamp) makes it a cross-tenant data
-- read and VOIDS the exemption. e2e/destinations.spec.ts asserts that property, so such an edit
-- fails the gate rather than silently widening an approved carve-out.
