-- flag-serving-and-prd-g · Sprint 1 hotfix — an explicit stale revision is an
-- application conflict, not PostgreSQL's retryable serialization-failure class.
--
-- SQLSTATE 40001 causes PostgREST's transaction machinery to retry/hold the request,
-- turning an ordinary stale UI submission into a timeout. P0001 is returned immediately;
-- the server action deliberately keeps the client-facing response generic.
...
BEGIN
  ...
  IF v_state.snapshot_version <> p_expected_snapshot_version THEN RAISE EXCEPTION 'flag snapshot version conflict' USING ERRCODE = 'P0001'; END IF;
