...
export default async function AgentKeysPage({ params }: { params: Promise<{ projectSlug: string }> }) {
  ...
  return (
    <main>
      ...
      <p>
        An agent write key lets your own agent <em>claim</em>, <em>resolve</em> and <em>dismiss</em> tasks in
        this project over MCP. It is the second half of a write: your agent also needs this project&apos;s
        connector URL, and both must belong to <strong>this same project</strong> or the write tools are not
        offered at all.
      </p>
      <p>
        This is deliberately <em>not</em> the connector token. That token is displayed openly on the install
        page — it travels through browser history, proxy logs and screenshots — so it identifies your project
        and authorizes reads, and nothing more. A key minted here is stored only as a hash, is never shown
        again, and can be revoked or expired independently.
      </p>
      <p>
        Nothing here can read or write anything outside this project, and nothing can change a task without a
        second confirmation step: your agent proposes a change, sees exactly what would happen, and applies it
