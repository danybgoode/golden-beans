  ...
  return (
    <ProductShell projectSlug={projectSlug}>
      <main>
        ...
        <p>
          Read-only operating evidence for bounded resilience and defensive-security exercises. Runtime gates
          may stay OFF while definitions, stopped runs, immutable impact snapshots and breaker decisions
          remain inspectable.
        </p>
        ...
        <table>
          ...
          <tbody>
            {view.targets.length === 0 ? (
              <tr>
                <td colSpan={5}>No scenario targets registered.</td>
              </tr>
            ) : (
        ...
        <table>
          ...
          <tbody>
            {view.runs.length === 0 ? (
              <tr>
                <td colSpan={8}>No scenario runs yet.</td>
              </tr>
            ) : (
              view.runs.map((run) => (
                <tr key={run.id}>
                  <td>
                    {run.scenarioKey} v{run.definitionVersion}
                    <br />
                  ...
                  <td>
                    {run.status} · r{run.revision}
                  </td>
                  <td>{run.requestCount}</td>
                  <td>
                    {run.successCount} ok / {run.failureCount} failed
                  </td>
        ...
        <table>
          ...
          <tbody>
            {view.securityResults.length === 0 ? (
              <tr>
                <td colSpan={6}>No defensive simulations recorded.</td>
              </tr>
            ) : (
              view.securityResults.map((result) => (
                <tr key={result.id}>
                  <td>
                    ...
                    <br />
                    <small>run {shortId(result.runId)}</small>
                  </td>
        ...
        <table>
          ...
          <tbody>
            {view.impacts.length === 0 ? (
              <tr>
                <td colSpan={6}>No impact snapshots captured.</td>
              </tr>
            ) : (
              view.impacts.map((impact) => (
                <tr key={impact.id}>
                  ...
                  <td>
                    {impact.scenarioKey} v{impact.scenarioVersion}
                    <br />
                    <small>run {shortId(impact.runId)}</small>
                  </td>
        ...
        <table>
          ...
          <tbody>
            {view.policies.length === 0 ? (
              <tr>
                <td colSpan={6}>No breaker policies configured.</td>
              </tr>
            ) : (
              view.policies.map((policy) => {
                const flag = policy.definition.flag
                const flagLabel =
                  typeof flag === 'object' && flag !== null && 'key' in flag ? String(flag.key) : 'unknown'
                return (
                  <tr key={policy.id}>
                    <td>
                      {policy.key}
                      <br />
                      <small>{shortId(policy.id)}</small>
                    </td>
                    <td>{flagLabel}</td>
                    <td>
        ...
        <table>
          ...
          <tbody>
            {view.trips.length === 0 ? (
              <tr>
                <td colSpan={6}>No breaker trips recorded.</td>
              </tr>
            ) : (
              view.trips.map((trip) => (
                <tr key={trip.id}>
                  ...
                  <td>
                    {trip.oldSnapshotVersion} → {trip.newSnapshotVersion}
                  </td>
                  <td>{trip.reason}</td>
                </tr>
              ))
