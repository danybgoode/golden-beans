...
// `experiment-governance-v2` shipped the immutable plan, the allocation table, the trust
// diagnostics, the freshness read and an **append-only decision ledger**, and the approved
// `experiment-ready` state draws none of them. The page now leads with the approved comparison and
// keeps ALL of this behind a disclosure, one keystroke away.
//
// Deleting a governance surface to satisfy a geometry assertion is the least defensible version of
// that trade — Sprint 4 made the same call for Destinations' two operational logs, for the same
// reason: no other surface offers it, and a design that never drew it was not asked to.
//
// ⚠️ It is moved **verbatim in substance**. The markup is unchanged, and the only edits are the ones
// forced by extraction: the `ProductShell` wrapper and the `h1` belong to the page now, so they are
// not here.
...
export function GovernanceDetail({ result, canManage }: { result: GovernedSuccess; canManage: boolean }) {
