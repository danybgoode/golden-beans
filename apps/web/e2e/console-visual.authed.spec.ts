...
test('every route matches the STRUCTURE of its approved state', async ({ page, browser }) => {
  ...
  const regressed = Object.keys(floor).filter((route) => floor[route] && !measured[route])
  const newlyMatching = Object.keys(measured).filter((route) => !floor[route])
  ...
  if (newlyMatching.length > 0) {
    console.log(
      `[structure] ${newlyMatching.length} route(s) now match their approved state and are not yet ` +
        `in the floor — commit apps/web/design-system/STATE-MATCH.json:\n  ${newlyMatching.join('\n  ')}`
    )
  }
