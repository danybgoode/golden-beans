...
### After the draft: three more real defects, and one at the very last step

Written while Sprint 1 was still in review; these happened afterwards and belong in the record.

- **Sprint 2's end-to-end spec found five defects on its first runs** — two Blocking ones Codex had
  just reported (a `set` resurrecting an OLD version; `--all-envs` computed from an unactivated
  DRAFT), and three nobody had: the **headline command returned 400** because the parser requires a
  description and `--description` is optional; re-run idempotency never fired because Postgres
  `jsonb` reorders object keys; and `--all-envs` went partial whenever *another* flag changed in the
  same environment. None was visible to 1,800 unit tests, because none of them ran the write route
  against the real control plane.
- **Two mutation checks reported nothing red — because they had broken the build.** The mutation
  left a symbol unused, lint failed inside `next build`, and no test ever ran. Redone with mutations
  that compile; all three guards went red.
- **The release would have crashed on a clean machine.** The CLI imported three SDK exports this epic
  added; the *published* SDK 0.4.0 had none of them. Inside the monorepo the workspace link resolves
  the local source, so every test passed. Caught by checking the real registry tarball before
  publishing; SDK 0.5.0 shipped first.
- **The review families ran out.** agy hit its quota, vibe could not finish a security review even at
...
- **Test the package as a stranger would install it, not as the monorepo resolves it.** Pack the
