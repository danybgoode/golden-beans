...
| Live engine proof | `LiveProofSection.tsx` — real TARS/North Star/A-B from the demo tenant, in-process (renamed `LiveEngineProof.tsx` in this epic, to match its export) | Nothing. It is **kept** and folded into §6 Proof (product-owner decision, 2026-08-12) |
| Pod Report proof | `PodsProofSection.tsx` + `lib/pod-report-query.ts`, investor lens, honest fallback (renamed `PodReportProof.tsx` in this epic, to match its export) | Nothing. The mockup's hardcoded `+3.1×/−68%` stat tiles are **not** shipped — the real computed figures render in that layout instead |
